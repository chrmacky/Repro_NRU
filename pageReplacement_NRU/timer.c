	...
	for (unsigned pid = 1; pid < MAX_PROCESSES; pid++)
	{
		if ((processTable[pid].valid) && (processTable[pid].pageTable != NULL))
			for (unsigned page = 0; page < processTable[pid].size; page++)
				if (processTable[pid].pageTable[page].present && !processTable[pid].pageTable[page].referenced) {
					//When a frame was not accessed during the last timer interval, we'll free the frame to use it where needed
					
					//printf("PID: %3u -> Try Freeing unused page %3u\n", pid, page);
					deAllocatePage(pid, page);
				}
